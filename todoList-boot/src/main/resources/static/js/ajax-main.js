...
function getTotalCount() {
    ...
    .then( response => {
        ...
        //reponse.test() : 응답 데이터를 문자열/숫자 형태로 변환한
...
function getCompleteCount() {
    ...
    //response.text() : 응답 데이터를 text형태로 변환(단일값만 가능하다.)

    //두 번쨰 then의 result
    //- 첫 번쟤 then 에서 text 로 변환된 응답 데이터
    ...
    .then(result => {
        ...
        //#completeCount 요소에 내용으로 result 값을 출력
...
addBtn.addEventListener("click" , () => {
    ...
    .then( temp => { //첫번째 then에서 반환된 값 중 변환된 text 를 temp에 저장
...
//비동기로 (ajax)로 할 일 상세 조회하는 함수
const selectTodo = (url) => {
    // 매개변수 url == '/ajax/detail?todoNo=10' 형태의 문자열이 넘어온다.
...
//popup layer 의 x버튼 (#popupClose) 클릭 시 닫기
...
deleteBtn.addEventListener("click",() => {
    ...
    fetch("/ajax/delete", {
        ...
        //데이터 하나를 전달해도 application/json 작성해줘야한다.
...
changeComplete.addEventListener("click", ()=> {
    ...
    const complete = popupComplete.innerText === 'Y' ? 'N' : 'Y'; //삼항연산자 이용
    ...
    fetch("/ajax/changeComplete", {
        method : "PUT",
        headers : {"Content-Type" : "application/json"},
        body : JSON.stringify(obj) // obj를 JSON 형태로 변경
    })
    .then(resp => resp.text() )
    .then(result => {
        ...
        if(result > 0){
            ...
            //selectTodo();
            //서버 부하를 줄이디 위해 상세 조회에서 Y/N 만 바꾸기
...
//상세 조회에서 수정 버튼 (update View) 클릭 시 
