...
public class ParameterController {
	 ...
	 * 
	 * HttpServletRequest : - 요청 클라이언트 정보, 제출된 파라미터 등을 저장한 객체 - 클라이언트 요청 시 생성
	 * 
	 * ArgumentResolver(전달 인자 해결사) - Spring의 Controller 메서드 작성 시 매개변수에 원하는 객체를 작성하면
	 ...
	 * 
	 * required : 입력된 name 속성값 파라미터 필수 여부 지정(기본값 true) -> required = true인 파라미터가
	 * 존재하지 않는다면 400 Bad Request 에러 발생
	 * 
	 * 
	 * defaultValue : 파라미터 중 일치하는 name 속성값이 없을 경우에 대입할 값 지정. -> required=false 인 경우
	 * 사용
	 * 
	 * 
	 */
	 ...
	 * 
	 * required 속성은 사용 가능하나, defaultValue 속성은 사용 불가
	 * 
	 */
	 ...
	 * 
	 * 전달받은 파라미터의 name 속성 값이 같이 사용되는 DTO의 필드명과 같으면 자동으로 Setter를 호출해서 필드에 값을 세팅해준다.
	 */
