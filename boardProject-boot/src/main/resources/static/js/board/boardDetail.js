/*좋아요 버튼(하트) 클릭 시 비동기로 좋아요 insert/delete 할건지 정해야 함. */
...
// 1) 로그인한 회원 번호 준비
//    --> session 에서 얻어오기(session 은 서버에서 관리하기 때문에 JS에서 바로 얻어올 방법이 없음)
...
document.querySelector("#boardLike").addEventListener("click", e =>{
    ...
    .then(count => {
        ...
        //7. 게시글 좋아요 수 수정
        e.target.nextElmentSibling.innerText = count;
...
if(updateBtn != null){ // 수정 버튼 존재 시
    ...
    updateBtn.addEventListener("click", () => {
        ...
        location.href = location.pathname.replace('board','editBoard')
                        + "/update"
                        + location.search; // +location.search; : 쿼리스트링 부분을 다 찾아줌
    });
