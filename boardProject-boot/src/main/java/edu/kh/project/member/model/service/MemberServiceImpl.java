...
@Service // 비즈니스 로직 처리 역할 명시 + Bean 으로 등록
public class MemberServiceImpl implements MemberService {
	...
	public Member login(Member inputMember) {
		...
		// bcryptPassword : $2a$10$sTGFdtV8ict5Ig4AmYqQAurAjQ7VDKqqjPjeWEYMT8gbRbydjdlfS
	...
	// 빠른 로그인 서비스
	@Override
	public Member login2(Member inputMember) {
		return mapper.login(inputMember.getMemberEmail());
	}
 ...
 * 
 * - 입력된 문자열 ( 비밀번호)에 salt를 추가한 후 암호화 하는 것.
 * 
 * ex) A회원 : 1234 -> $12!asdfg
 * 
 * ex) B회원 : 1234 -> $12!sdkjd
 * 
 * -비밀번호 확인 방법 ->BCrtptPasswordEncoder.matches(평문 비밀번호, 암호화된 비밀번호)
 * 
 * -> 평문 비밀번호와
 * 
 * 암호화된 비밀번호가 같은 경우 true 아니면 false 반환
 * 
 * *로그인/ 비밀번호 변경 / 탈퇴 등 비밀번호가 사용되는 로직(입력되는 경우)
 * 
 ...
 * 
 * ex) B회원 : 1234 -> 암호화 : abcd( 암호화 시 변경된 내용이 같음)
 * 
 * 
 * 
 */
