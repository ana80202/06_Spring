...
public class TestWebsocketHandler extends TextWebSocketHandler {
	...
	// SessionHandshakeIntercepter 가 가로챈
	// 연결한 클라이언트의 HttpSession을 가지고 있음
	// (attributes 에 추가한 값)

	// set 들어온 순서 유지는 되지 않지만 중복되지 않는다!
	// 동기화된 set 을 생성
	// 동기화 : (일치시킨다는 뜻을 동기화 x 비동기 동기할떄 그거임)
	private Set<WebSocketSession> sessions = Collections.synchronizedSet(new HashSet<>());
	// -> 여러가지 스레드가 동작하는 환경에서 하나의 컬렉션에 여러 스레드가 접근하여 의도치 않은 문제가 발생되지 않게
	// 하기 위해 동기화를 진행하여 스래드가 순서대로 한 컬렉션에 접근할 수 있도록 변경.

	// 클라이언트와 연결이 완료되고, 통신할 준비가 되면 실행
	@Override
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {

		// 연걀된 클라이언트의 WebsocketSession 정보를 set에 추가
