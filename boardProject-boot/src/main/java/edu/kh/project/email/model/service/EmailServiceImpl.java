...
public class EmailServiceImpl implements EmailService {
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// MimeMessageHelper :
			// Spring에서 제공하는 메일 발송 도우미(간단하게 보낼수 있음 + 타임리프 이용 가능)
			...
			// HTML 코드 해석 여부 true(innerHTML 해석 할지 말지)
		...
		// 1) 해당 이메일이 DB에 존재하는 경우가 있을 수 있음
		// 수정(update) 을 먼저 진행
		// -> 1반환 == 업데이트 성공 == 이미 존재해서 인증번호 변경
	...
	// html 파일을 읽어와 String 으로 변환해주는 method (타임리프 적용)
 ...
 * 
 * - SMTP(Simple MAil Transfer Protocol, 간단한 메일 전송 규약) --> 이메일 메시지를 보내고 받을 때
 * 사용하는 약속(규약, 방법)
 * 
 * - Google SMTP Java Mail Sender -> Google SMTP -> 대상에게 이메일 전송 -Java MAil
 * Sender에 Google SMTP 이용 설정 추가
 * 
