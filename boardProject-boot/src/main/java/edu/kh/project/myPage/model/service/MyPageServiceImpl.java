...
public class MyPageServiceImpl implements MyPageService {
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws IOException {
		 ...
		 * 3) 만약에 파일 저장 실패 시 
		 * 	  -> 예외 발생
		 *    -> @Transactional 이용해서 rollback 수행
		 * 
		 * */
		...
		// C:\\uploadFiles\\test\20204041711603_00001.png
		// -> CheckedException 발생 -> 예외 처리 필수

		// @Transactional 은 RuntimeException(UncheckedException 대표) 만 처리
		// -> rollbackFor 속성 이용해서
		// 롤백할 예외 범위를 수정
	...
	public int profile(MultipartFile profileImg, Member loginMember) throws Exception {
		...
		// 업로드한 이미지가 있을 경우
		// - 있을 경우 : 수정할 경로 조합(클라이언트에서 접근할 수 있는 경로 + 리네임한파일명)
